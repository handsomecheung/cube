...
use crate::chunk::{
    compress, pack_data, Chunk, ChunkHeader, DEFAULT_PAYLOAD_SIZE, HEADER_SIZE,
};
...
pub fn encode_file_to_images(
    ...
    let (chunks, effective_size, filename) = prepare_chunks(input_path, chunk_size, pixel_scale, 1.5)?;
...
pub fn encode_file_for_terminal(
    ...
    loop {
        ...
        let packet_size = (current_size - HEADER_SIZE) as u16;
...
pub fn encode_file_to_gif(
    ...
    let (chunks, effective_size, _filename) = prepare_chunks(input_path, chunk_size, pixel_scale, 1.5)?;
